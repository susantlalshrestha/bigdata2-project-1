...
```{r}
year_counts <- clean_dfID %>%
  mutate(Year = as.integer(Year)) %>%
  group_by(Year) %>%
  summarize(Count = n())

trend_plot_year <- ggplot(data = year_counts, aes(x = Year, y = Count)) +
  geom_line() +
  labs(title = "Trend of Accidents by Year", x = "Year", y = "Number of Accidents") +
...
clean_dfID$Longitude[is.na(clean_dfID$Longitude)] <- mean(clean_dfID$Longitude, na.rm = TRUE)
clean_dfID$Latitude[is.na(clean_dfID$Latitude)] <- mean(clean_dfID$Latitude, na.rm = TRUE)
...
train_conf_matrix <- confusionMatrix(train_predictions, as.factor(data_train[, target]))
...
test_conf_matrix <- confusionMatrix(test_predictions, as.factor(data_test[, target]))
accuracy <- train_conf_matrix$overall["Accuracy"]
